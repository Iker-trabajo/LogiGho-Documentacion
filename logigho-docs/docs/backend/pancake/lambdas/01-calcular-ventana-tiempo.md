...
| --------------------- | ------------------------ | ---------------------------- | -------------------------------------------------------- |
| `cierre_dia_anterior` | Ayer 00:00:00 (Colombia) | Ayer **23:59:59** (Colombia) | Consolidar el día anterior completo (corrida de las 7am) |
| `intradia_actual`     | Hoy 00:00 (Colombia)     | Momento de la ejecucion      | Foto del día en curso (corridas 9am / 2pm / 5pm)         |
...
FunctionHandler (Function.cs)
  -> CalcularVentanaUseCase.Ejecutar(entrada)
       ...
       -> Según tipo_calculo:
            cierre_dia_anterior -> [ayer 00:00, hoy 23:59)
            intradia_actual     -> [hoy 00:00,  hora ejecucion)
...
| 2026-07-27 | Iker Acevedo | Fix `cierre_dia_anterior`: `until` cambiado de `00:00:00` del día siguiente a `**23:59:59` del mismo día reportado** (helper `FinDeDiaColombia`). El `00:00` del día siguiente pertenece al día siguiente y Pancake arrastraba gasto del otro día. Ahora idéntico al cálculo del equipo. |
